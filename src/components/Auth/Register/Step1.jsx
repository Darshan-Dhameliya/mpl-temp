...
import PasswordFied from "@/components/@core/PasswordFied";
...
export default function Step1({ openLogin, nextStep }) {
  const dispatch = useDispatch();
  const handleSubmit = async (dto, { setSubmitting }) => {
    setSubmitting(true);
    if (dto.loginType === 0) {
      delete dto.phone;
    }
    dto.dob = moment(dto.dob).format("DD-MM-YYYY");
    const { data, message, statusCode } = await Api.auth.register(dto);
    if (statusCode === 200) {
    ...
    <>
      ...
      >
        {({
          ...
          >
            <div className="self-end text-[#8D93A9]">step 1/2</div>
            <div className="flex flex-row items-center justify-center gap-6">
              <div
                onClick={() => setFieldValue("loginType", 0)}
                className={`${
                  values.loginType === 0
                    ? "bg-[#3D4554] text-white"
                    : "bg-[#212530] text-[#8D93A9]"
                } cursor-pointer hover:bg-[#3D4554] hover:text-white w-1/2 p-3 rounded-lg  flex flex-row justify-center items-center gap-2`}
              >
                <EmailIcon height={18} width={18} /> Email
              </div>
              <div
                onClick={() => setFieldValue("loginType", 1)}
                className={`${
                  values.loginType === 1
                    ? "bg-[#3D4554] text-white"
                    : "bg-[#212530] text-[#8D93A9]"
              ...
              >
                <PhoneIcon height={18} width={18} /> Phone
              </div>
            </div>
            {values.loginType === 0 ? (
              <TextField
            ...
            <div>
              <TextField
                ...
                onChange={handleChange}
              />
              <div className="text-[#626778] ml-3 mt-1 block text-sm">
            ...
            <div class="flex items-center">
              ...
              />
              <label for="agree" class="ml-2 text-red-700 text-sm">
                I agree I agree to the con rm I am at least years old
              </label>
            </div>
            <div class="flex items-center ">
              <input
                ...
                checked
                name="notification"
                class="w-4 h-4 checked:bg-transparent border-gray-400 rounded "
              />
              <label for="notification" class="ml-2 text-white text-sm">
                I agree to receive marketing promotions from CASINO.
              </label>
            </div>
            <Button
